...
test('check default tags visible', async ({ page }) => {
  await expectTagsVisible(page, ['Learn', 'Create', 'Engage']);
});

test('remove default tags', async ({ page }) => {
  await openTagsEditor(page);
  await removeSelectedTag(page, 'Learn');
  await closeTagsEditor(page);

  await expectTagsVisible(page, ['Create', 'Engage']);
});

test('allow create tag', async ({ page }) => {
  await openTagsEditor(page);
  await searchAndCreateTag(page, 'Test');
  await closeTagsEditor(page);

  await expectTagsVisible(page, ['Learn', 'Create', 'Engage', 'Test']);
});
